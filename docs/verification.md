...
新增申请和棋：经对方同意后结束本局，各得 0.5 分；拒绝或超时继续。两端均需 2.0.1，握手会检查和棋功能支持。

49 项自动测试通过；静态分析无问题。覆盖同机和棋拒绝/同意、双方半分、和棋后同房重开与双端同步。版本为 2.0.1+3，沿用原应用标识及开发签名。真实手机覆盖更新及蓝牙互通仍待验收。

Release APK 构建成功，53,324,760 字节；签名验证通过，与 1.0 / 2.0 证书一致。SHA-256：`3a69fa2bd16796fab8f5a2053554b12638c5e28cbe5021c87ea361e7c28284c1`。
